...
获得类名、索引
'''
def get_info(classes_map):
    with open(classes_map, encoding='utf-8') as f:
        datas = f.readlines()
    classes_names = []
    indexs = []
    for data in datas:
        data = data.strip()
        if data:
            # 【核心修改】使用 rsplit 从右边切分一次
            # 这样 "Standard View 0" 会被切分为 ["Standard View", "0"]，完美解决！
            try:
                name, index = data.rsplit(' ', 1)
                classes_names.append(name)
                indexs.append(int(index))
            except ValueError:
                print(f"[警告] 无法解析类别行: {data}")
                continue
    return classes_names, indexs
