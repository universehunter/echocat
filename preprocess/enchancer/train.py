...
def exponential_decay(decay_rate: float = 0.5, decay_steps: int = 5e3, staircase: bool = True):
...
def main(cfg: DictConfig) -> None:
    ...
    # Flatten 2-level config for wandb
    d_cfg = {}
    ...
    # 如果数据集为空，直接报错
    assert len(dataset_train) > 0, "Train dataset is empty! Check your data path or config."
    if cfg.training.validate:
        assert len(dataset_valid) > 0, "Validation dataset is empty! Check your data path or config."
    ...
    try:
        ...
        print(f"First batch keys: {list(first_batch.keys()) if isinstance(first_batch, dict) else 'not a dict'}")
        if isinstance(first_batch, torch.Tensor):
            print(f"First batch shape: {first_batch.shape}")
    except Exception as e:
        print("Failed to get first batch from loader_train")
        traceback.print_exc()
        return
    ...
    if cfg.optim.scheduler == "cosine":
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, cfg.training.steps, eta_min=cfg.optim.eta_min)
    else:
