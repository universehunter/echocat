...
class AbstractNoiseDataset(Dataset, ABC):
    ...
    def __getitem__(self, i: int) -> Dict[str, Any]:
        ...
        if self.standardize:
            # 如果 mean/std 为空，则按 image 自己计算
            mean_tensor = torch.from_numpy(self.mean) if self.mean is not None else ret["image"].mean()
            std_tensor = torch.from_numpy(self.std) if self.std is not None else ret["image"].std()

            ret["image"], ret["mean"], ret["std"] = self._standardize(
                ret["image"], mean_tensor, std_tensor
            )

            if self.ground_truth is not None:
                ret["ground_truth"], _, _ = self._standardize(
                    ret["ground_truth"], ret["mean"], ret["std"]
                )
        else:
            # 数据已经标准化或不标准化
