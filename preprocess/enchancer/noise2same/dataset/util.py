...
def mask_like_image(
    ...
    box_size = np.round(np.power(100 / mask_percentage, 1 / n_dim)).astype(np.int64)
...
class ResizeIfSmall(albu.DualTransform):
    ...
    def apply(self, img, **params):
        h, w = img.shape[:2]
        new_h = max(h, self.height)
        new_w = max(w, self.width)
    ...
    def apply_to_mask(self, mask, **params):
        # mask might be HxW or HxWx1; use nearest interpolation to preserve labels
        h, w = mask.shape[:2]
        new_h = max(h, self.height)
        new_w = max(w, self.width)
        if new_h != h or new_w != w:
...
def training_augmentations_2d(crop: int = 64):
    return albu.Compose([
            ResizeIfSmall(height=crop, width=crop, always_apply=True, p=1.0),
            albu.RandomCrop(height=crop, width=crop, p=1.0),
            albu.RandomRotate90(p=0.5),
            albu.HorizontalFlip(p=0.5),
        ], 
        additional_targets={"ground_truth": "image"}  # ground_truth 用 apply
        )
...
def test_augmentations_2d():
    return albu.Compose([], additional_targets={"ground_truth": "image"})  # ground_truth 用 apply)
